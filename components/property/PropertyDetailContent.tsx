          ...
          <div className="lg:col-span-8 space-y-4">
            <div className="relative aspect-[16/10] overflow-hidden rounded-xl shadow-sm group">
              ...
              <div className="absolute top-4 left-4 flex gap-2">
                {property.featured && (
                  <span className="bg-mosque text-white text-xs font-medium px-3 py-1.5 rounded-full uppercase tracking-wider shadow-sm">{t.propertyDetail.featured}</span>
                )}
                {property.is_new && (
                  <span className="bg-white/90 backdrop-blur text-nordic text-xs font-medium px-3 py-1.5 rounded-full uppercase tracking-wider shadow-sm">{t.propertyDetail.isNew}</span>
                )}
              ...
              <button className="absolute bottom-4 right-4 bg-white/90 hover:bg-white text-nordic px-4 py-2 rounded-lg text-sm font-medium shadow-lg backdrop-blur transition-all flex items-center gap-2">
                ...
                {t.propertyDetail.viewAllPhotos}
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-8">
            <div className="bg-white p-8 rounded-xl shadow-sm border border-mosque/5">
              <h2 className="text-lg font-semibold mb-6 text-nordic">{t.propertyDetail.propertyFeatures}</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                <div className="flex flex-col items-center justify-center p-4 bg-mosque/5 rounded-lg border border-mosque/10">
                  ...
                  <span className="text-xs uppercase tracking-wider text-nordic/50">{t.propertyDetail.squareMeters}</span>
                </div>
                <div className="flex flex-col items-center justify-center p-4 bg-mosque/5 rounded-lg border border-mosque/10">
                  ...
                  <span className="text-xs uppercase tracking-wider text-nordic/50">{t.propertyDetail.bedrooms}</span>
                </div>
                <div className="flex flex-col items-center justify-center p-4 bg-mosque/5 rounded-lg border border-mosque/10">
                  ...
                  <span className="text-xs uppercase tracking-wider text-nordic/50">{t.propertyDetail.bathrooms}</span>
                </div>
                <div className="flex flex-col items-center justify-center p-4 bg-mosque/5 rounded-lg border border-mosque/10">
                  ...
                  <span className="text-xs uppercase tracking-wider text-nordic/50">{t.propertyDetail.garage}</span>
                </div>
              </div>
            </div>

            <div className="bg-white p-8 rounded-xl shadow-sm border border-mosque/5">
              <h2 className="text-lg font-semibold mb-4 text-nordic">{t.propertyDetail.aboutHome}</h2>
              ...
              <button className="mt-4 text-mosque font-semibold text-sm flex items-center gap-1 hover:gap-2 transition-all">
                {t.propertyDetail.readMore}
                <span className="material-icons text-sm">arrow_forward</span>
              </button>
            </div>

            <div className="bg-white p-8 rounded-xl shadow-sm border border-mosque/5">
              <h2 className="text-lg font-semibold mb-6 text-nordic">{t.propertyDetail.amenities}</h2>
            ...
            <div className="bg-mosque/5 p-6 rounded-xl border border-mosque/10 flex flex-col sm:flex-row items-center justify-between gap-6">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-semibold text-nordic">{t.propertyDetail.estimatedPayment}</h3>
                  <p className="text-sm text-nordic/60">{t.propertyDetail.startingFrom} <strong className="text-mosque">{property.price_formatted.replace('/mo', '')}/mo</strong> {t.propertyDetail.withDownPayment}</p>
                </div>
              </div>
